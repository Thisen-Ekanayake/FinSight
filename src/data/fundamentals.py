...
from src.data.schemas import FundamentalMetric
...
def _from_edgar(ticker: str, metrics: list[str], *, annual_only: bool) -> dict[str, FundamentalMetric]:
    ...
    results: dict[str, FundamentalMetric] = {}
    for metric in metrics:
        for concept in METRIC_CONCEPTS.get(metric, []):
            ...
            latest = candidates[-1]
            results[metric] = FundamentalMetric(
                ticker=ticker.upper(),
                metric=metric,
                value=latest["value"],
                unit=latest["unit"],
                period=f"{latest['fiscal_year']} {latest['fiscal_period']}".strip(),
                as_of=latest["period_end"],
                provider="edgar_xbrl",
                # The accession number IS the citation. No LLM involved.
                source_id=latest["accession_no"],
                source_url=EDGAR_ACCESSION_URL.format(cik=cik, form=latest["form_type"]),
            )
    ...
    logger.info("Fundamentals: %s -> %d/%d metrics from EDGAR XBRL", ticker, len(results), len(metrics))
    return results


def _from_yfinance(ticker: str, metrics: list[str]) -> dict[str, FundamentalMetric]:
...
def get_fundamentals(
    ...
    *,
    ...
    """
    Fetch fundamental metrics through the trust-ordered provider chain.
    ...
    annual_only : bool, default False
        Restrict EDGAR facts to 10-K filings, for like-for-like annual
        comparisons rather than mixing in quarterly figures.
    ...
    result = run_chain(
        "fundamentals",
        {
            "edgar_xbrl": lambda: _from_edgar(ticker, metrics, annual_only=annual_only),
